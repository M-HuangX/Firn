...
export interface TheaterInputNodeData extends Record<string, unknown> {
  label: string;
  state: NodeState;
  ticker?: string;
}

function TheaterInputNodeComponent({ data }: NodeProps) {
  const { label, state, ticker } = data as unknown as TheaterInputNodeData;

  return (
    <div
      className={cn(
        "px-5 py-4 rounded-xl border-2 min-w-[120px] transition-all duration-500",
        state === "complete"
...
export const TheaterInputNode = memo(TheaterInputNodeComponent);
